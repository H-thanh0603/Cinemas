...
export function ResetForm({ token }: { token: string }) {
  ...
  return (
    <form onSubmit={submit} className="mt-6 space-y-4">
      ...
      <button disabled={loading} className="w-full rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-on-primary disabled:opacity-50">{loading ? "Đang lưu..." : "Đặt lại mật khẩu"}</button>
